...
# Data input settings
parser.add_argument('--cfgs_file', default='cfgs/anet.yml', type=str,
...
parser.add_argument('--val_data_folder', default='validation',
                    help='validation data folder')
parser.add_argument('--densecap_eval_file',
                    default='/z/subsystem/densevid_eval/evaluate.py')
...
parser.add_argument('--max_prop_num', default=500, type=int,
                    help='the maximum number of proposals per video')
parser.add_argument('--min_prop_num', default=50, type=int,
                    help='the minimum number of proposals per video')
        ...
        if args.dataset == 'anet':
            for line in f:
                ...
                frame_to_second[vid_name] = float(
                    vid_dur)*int(float(vid_frame)*1./int(float(vid_dur))*args.sampling_sec)*1./float(vid_frame)
            # a missing video in anet
            frame_to_second['_0CqozZun3U'] = args.sampling_sec
        elif args.dataset == 'yc2':
            import math
            for line in f:
                vid_name, vid_dur, vid_frame = [l.strip() for l in line.split(',')]
                frame_to_second[vid_name] = float(
                    vid_dur)*math.ceil(float(vid_frame)*1./float(vid_dur)*args.sampling_sec)*1./float(vid_frame)  # for yc2
        else:
    ...
    for data in loader:
        ...
        with torch.no_grad():
            ...
            if video_prefix[0].split('/')[-1] not in frame_to_second:
                ...
                print("cannot find frame_to_second for video {}".format(
                    video_prefix[0].split('/')[-1]))
            sampling_sec = frame_to_second[video_prefix[0].split(
                '/')[-1]]  # batch_size has to be 1
...
def eval_results(densecap_result, prop_result, args):
    ...
    subprocess.Popen(["python2", args.densecap_eval_file, "-s",
                      os.path.join(args.result_root, 'densecap_' +
                                   args.val_data_folder+'_' + args.id + '.json'),
                      "-v", "-r"] +
    ...
    anet_proposal = ANETproposal(args.dataset_file,
                                 os.path.join(args.result_root, 'prop_' +
                                              args.val_data_folder+'_' + args.id + '.json'),
