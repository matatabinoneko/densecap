...
class ActionPropDenseCap(nn.Module):
    ...
    def forward(self, x, s_pos, s_neg, sentence,
        ...
        for i, kernel in enumerate(self.prop_out):
            ...
            if kernel_size <= vis_feat.size(-1):
                ...
                anchor_l = Variable(torch.FloatTensor(
        ...
        pred_score = Variable(torch.FloatTensor(
            np.zeros((sample_each*B, 2))).type(dtype))
        gt_score = Variable(torch.FloatTensor(np.zeros((sample_each*B, 2))).type(dtype))
        pred_offsets = Variable(torch.FloatTensor(
            np.zeros((sample_each*B, 2))).type(dtype))
        gt_offsets = Variable(torch.FloatTensor(
            np.zeros((sample_each*B, 2))).type(dtype))
            ...
            for i in range(sample_each):
                ...
                if i == pred_index:  # only need once, since one sample corresponds to one sentence only
                    ...
                    if self.learn_mask:
                        ...
                        pe_locs[B*2 + b] = Variable(torch.Tensor(
                            [max(0, math.floor(anc_cen - anc_len / 2.))]).type(dtype))
                        pe_locs[B*3 + b] = Variable(torch.Tensor(
                            [min(T, math.ceil(anc_cen + anc_len / 2.))]).type(dtype))
    ...
    def inference(self, x, actual_frame_length, sampling_sec,
        ...
        for b in range(B):
            ############################ ここを修正　############################
            # 0 indexが確信度 ここをランダムな値に置換する
            crt_pred = prop_all.data[b]

            # original_frame_lenを超えない様にランダムに洗濯
            ...
            for nms_thresh in nms_thresh_set:
                for prop_idx in range(nproposal):
                    ...
                    original_frame_len = actual_frame_length[b].item() + sampling_sec*2
                    pred_start_w = crt_pred_cen[sel_idx[prop_idx]
                                                ] - crt_pred_len[sel_idx[prop_idx]] / 2.0
                    pred_end_w = crt_pred_cen[sel_idx[prop_idx]
                                              ] + crt_pred_len[sel_idx[prop_idx]] / 2.0
                    ...
                    if not hasoverlap:
                        ...
                        win_start = math.floor(
                            max(min(pred_start, min(original_frame_len, T)-1), 0))
                        win_end = math.ceil(
                        ...
                        if self.learn_mask:
                            ...
                            gate_scores.append(torch.Tensor(
                                [crt_pred[0, sel_idx[prop_idx]]]).type(dtype))
            ...
            print(window_mask.size(), cap_batch)
            for sent_i in range(math.ceil(window_mask.size(0)/cap_batch)):
                ...
                # print(sent_i, batch_start, batch_end)
                # ここをbatch_start=i, batch_end=i+1になる様にwindow_maskを修正
            ...
            print('Processing time for tIoU: {:.2f}, mask: {:.2f}, caption: {:.2f}'.format(
                mid1_t-start_t, mid2_t-mid1_t, end_t-mid2_t))

        return all_proposal_results

    def random_inference(self, x, actual_frame_length, sampling_sec,
                         min_prop_num, max_prop_num,
                         min_prop_num_before_nms, pos_thresh, stride_factor,
                         gated_mask=False):
        B, T, _ = x.size()
        ...
        for b in range(B):
            ...
            # 0 indexが確信度 ここをランダムな値に置換する
            crt_pred = prop_all.data[b]
            crt_pred[0] = torch.ones_like(crt_pred[0])

            # original_frame_lenを超えない様にランダムに洗濯
            crt_pred_cen = pred_cen.data[b]
            # crt_pred_cen = torch.rand(crt_pred_cen.size()) * original_frame_len

            # ここをlenを1にする
            crt_pred_len = pred_len.data[b]
            crt_pred_len = torch.ones_like(crt_pred_len)
            ####################################################################
            pred_masks = []
            batch_result = []
            crt_nproposal = 0
